...
fn process_trial(
    ...
    // Axis independent buffers
    let mut p_sys_x_buf = Vec::new();
    let mut p_sys_y_buf = Vec::new();
    let mut p_sys_z_buf = Vec::new();
    let mut com_x_buf = Vec::new();
    let mut com_y_buf = Vec::new();
    let mut com_z_buf = Vec::new();

    let mut last_invalid_breath_time = -f64::INFINITY;

    for i in 0..len {
        let t = times[i];
        let dt = if i < len - 1 { times[i + 1] - t } else { dt_default };
        ...
        // Push separated axes and CoM state
        p_sys_x_buf.push(res.power_sys_x);
        p_sys_y_buf.push(res.power_sys_y);
        p_sys_z_buf.push(res.power_sys_z);
        com_x_buf.push(res.com_pos.x);
        com_y_buf.push(res.com_pos.y);
        com_z_buf.push(res.com_pos.z);
        ...
        if r_is_active && !r_was_active {
            if let Some(start_idx) = last_r_strike_idx {
                ...
                if slice_len <= p_exo_l_buf.len() && slice_len <= p_exo_r_buf.len() {
                    ...
                    let sys_x_slice = &p_sys_x_buf[p_sys_x_buf.len() - slice_len..];
                    let sys_y_slice = &p_sys_y_buf[p_sys_y_buf.len() - slice_len..];
                    let sys_z_slice = &p_sys_z_buf[p_sys_z_buf.len() - slice_len..];
                    let com_x_slice = &com_x_buf[com_x_buf.len() - slice_len..];
                    let com_y_slice = &com_y_buf[com_y_buf.len() - slice_len..];
                    let com_z_slice = &com_z_buf[com_z_buf.len() - slice_len..];
                    ...
                    cycles.push(GaitCycleRecord {
                        ...
                        mass_kg: res.mass,
                        is_valid,
                        work_exo_pos: trapz_1d(&pos(exo_l_slice), dt) + trapz_1d(&pos(exo_r_slice), dt),
                        work_exo_neg: trapz_1d(&neg(exo_l_slice), dt) + trapz_1d(&neg(exo_r_slice), dt),
                        ...
                        work_sys_x_pos: trapz_1d(&pos(sys_x_slice), dt),
                        work_sys_x_neg: trapz_1d(&neg(sys_x_slice), dt),
                        work_sys_y_pos: trapz_1d(&pos(sys_y_slice), dt),
                        work_sys_y_neg: trapz_1d(&neg(sys_y_slice), dt),
                        work_sys_z_pos: trapz_1d(&pos(sys_z_slice), dt),
                        work_sys_z_neg: trapz_1d(&neg(sys_z_slice), dt),
                        com_amp_x: get_amplitude(com_x_slice),
                        com_amp_y: get_amplitude(com_y_slice),
                        com_amp_z: get_amplitude(com_z_slice),
                    });
                }
            }
        ...
        if i > 0 && vo2[i] > 0.0 && (vo2[i] - vo2[i - 1]).abs() > 1e-3 {
            ...
            if !physio_valid { last_invalid_breath_time = t; }
...
fn main() {
    ...
    println!("\nProcessing {} trials using 6 parallel threads...", files.len());
    ...
    if !all_cycles.is_empty() {
        let mut df_cycles = df!(
            ...
            "work_sys_x_pos" => all_cycles.iter().map(|c| c.work_sys_x_pos).collect::<Vec<_>>(),
            "work_sys_x_neg" => all_cycles.iter().map(|c| c.work_sys_x_neg).collect::<Vec<_>>(),
            "work_sys_y_pos" => all_cycles.iter().map(|c| c.work_sys_y_pos).collect::<Vec<_>>(),
            "work_sys_y_neg" => all_cycles.iter().map(|c| c.work_sys_y_neg).collect::<Vec<_>>(),
            "work_sys_z_pos" => all_cycles.iter().map(|c| c.work_sys_z_pos).collect::<Vec<_>>(),
            "work_sys_z_neg" => all_cycles.iter().map(|c| c.work_sys_z_neg).collect::<Vec<_>>(),
            "com_amp_x" => all_cycles.iter().map(|c| c.com_amp_x).collect::<Vec<_>>(),
            "com_amp_y" => all_cycles.iter().map(|c| c.com_amp_y).collect::<Vec<_>>(),
            "com_amp_z" => all_cycles.iter().map(|c| c.com_amp_z).collect::<Vec<_>>(),
        )
        .unwrap();

        let cycles_file = std::fs::File::create("../exported_pogensee/gait_cycles.parquet").unwrap();
        ParquetWriter::new(cycles_file).finish(&mut df_cycles).unwrap();
    ...
    if !all_breaths.is_empty() {
        ...
        ParquetWriter::new(breaths_file).finish(&mut df_breaths).unwrap();
