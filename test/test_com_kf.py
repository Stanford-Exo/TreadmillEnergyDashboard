...
class TestComKalmanFilter(unittest.TestCase):
    ...
    def test_tilt_and_mass_convergence(self):
        """Simulate a stationary block on a tilted surface to verify convergence."""
        true_mass = 75.0
        true_theta_x = 0.02
        true_theta_y = -0.01
        
        R = np.array([
            [1.0, 0.0, true_theta_y],
            [0.0, 1.0, -true_theta_x],
            [-true_theta_y, true_theta_x, 1.0]
        ])
        F_global = np.array([0.0, 0.0, true_mass * 9.81])
        ...
        self.assertTrue(abs(kf.tilt_angles[1] - true_theta_y) < 0.005)
