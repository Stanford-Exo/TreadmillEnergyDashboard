...
def get_rotated_foot_marker(angle_deg):
    ...
    R = np.array([[c, -s], [s, c]])
    return Path(
        np.dot(verts, R.T), [Path.MOVETO] + [Path.LINETO] * 7 + [Path.CLOSEPOLY]
    )
...
def animate_trial(trial_name, df_trial, save, output_dir):
    ...
    def update(frame_idx):
        ...
        progress_line = ax_top.axvline(
            current_time_min, color=NOTION_TEXT, linestyle=":", linewidth=1.5, zorder=5
        )
        ...
        ax.fill_between(
            ...
            z,
            pos_ref_exo,
            color=NOTION_REF_EXO,
            alpha=0.9,
            linewidth=0,
            label="Stance Leg Exo",
        )
        ax.fill_between(
            stride_pct,
            pos_ref_exo,
            pos_ref_ach,
            color=NOTION_REF_ACH,
            alpha=0.9,
            linewidth=0,
            label="Stance Leg Achilles (Passive)",
        )
        ax.fill_between(
            stride_pct,
            pos_ref_ach,
            pos_ref_mus,
            color=NOTION_REF_MUS,
            alpha=0.9,
            linewidth=0,
            label="Stance Leg Muscle (Active)",
        )
        ...
        ax.fill_between(
            ...
            pos_ref_mus,
            pos_con_exo,
            facecolor=NOTION_CON_EXO_FACE,
            edgecolor=NOTION_CON_EXO_EDGE,
            linewidth=0.5,
            label="Swing Leg Exo",
        )
        ax.fill_between(
            ...
            pos_con_exo,
            pos_con_ach,
            facecolor=NOTION_CON_ACH_FACE,
            edgecolor=NOTION_CON_ACH_EDGE,
            linewidth=0.5,
            label="Swing Leg Achilles (Passive)",
        )
        ax.fill_between(
            stride_pct,
            pos_con_ach,
            pos_con_mus,
            facecolor=NOTION_CON_MUS_FACE,
            edgecolor=NOTION_CON_MUS_EDGE,
            linewidth=0.5,
            label="Swing Leg Muscle (Active)",
        )

        # Plot Negative Solid Fills
        ax.fill_between(
            stride_pct, z, neg_ref_exo, color=NOTION_REF_EXO, alpha=0.9, linewidth=0
        )
        ax.fill_between(
            stride_pct,
            neg_ref_exo,
        ...
        def label_blocks(
            ...
            for i in range(len(boundaries) - 1):
                ...
                y_exo = (
                    pos_exo_base[peak_idx] + p_e / 2
                    if p_e > 0
                    else neg_exo_base[peak_idx] + p_e / 2
                )
                y_ach = (
                    pos_ach_base[peak_idx] + p_a / 2
                    if p_a > 0
                    else neg_ach_base[peak_idx] + p_a / 2
                )
                y_mus = (
                    pos_mus_base[peak_idx] + p_m / 2
                    if p_m > 0
                    else neg_mus_base[peak_idx] + p_m / 2
                )
        ...
        label_blocks(
            ...
            ref_mus,
            pos_exo_base=z,
            pos_ach_base=pos_ref_exo,
            pos_mus_base=pos_ref_ach,
            neg_exo_base=z,
            neg_ach_base=neg_ref_exo,
            neg_mus_base=neg_ref_ach,
            t_color_exo=TEXT_COLOR_EXO,
            t_color_ach=TEXT_COLOR_ACH,
            t_color_mus=TEXT_COLOR_MUS,
        )
        ...
        label_blocks(
            ...
            con_ach,
            con_mus,
            pos_exo_base=pos_ref_mus,
            pos_ach_base=pos_con_exo,
            pos_mus_base=pos_con_ach,
            neg_exo_base=neg_ref_mus,
            neg_ach_base=neg_con_exo,
            neg_mus_base=neg_con_ach,
            t_color_exo=TEXT_COLOR_EXO,
            t_color_ach=TEXT_COLOR_ACH,
            t_color_mus=TEXT_COLOR_MUS,
        )
        ...
        ax.scatter(
            ...
            -0.1,
            s=450,
            marker=get_rotated_foot_marker(-25),
            facecolors=NOTION_TEXT,
            edgecolors="none",
        ...
        ax.scatter(
            ...
            -0.1,
            s=450,
            marker=get_rotated_foot_marker(30),
            facecolors="none",
            edgecolors=NOTION_SUBTEXT,
            linestyles="--",
            linewidths=1.2,
            transform=ax.get_xaxis_transform(),
            clip_on=False,
        ...
        ax.set_ylabel(
            "Mechanical Power (W)", fontsize=11, fontweight="bold", color=NOTION_TEXT
        )
        ax.text(
            0.5,
            -0.3,
