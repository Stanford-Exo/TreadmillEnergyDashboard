...
class StrideAnalyzer:
    ...
    def __init__(
        self, contact_threshold=30.0, foot_roll_length=0.254, override_belt_speed=None
    ):
        ...
        self.current_belt_speed = (
            override_belt_speed if override_belt_speed is not None else 1.25
        )
        self.belt_velocity = np.array(
            [-self.current_belt_speed, 0.0, 0.0]
        )  # 3D vector for frame translation
                    ...
                    if 0.4 < stride_dur < 1.5 and is_clean:
                        ...
                        if len(self.toe_off_times[foot]) > 0:
                            ...
                            if last_toe_off > self.active_stance_start_time[foot]:
                                stance_dur = (
                                    last_toe_off - self.active_stance_start_time[foot]
                                )
                                self.metrics["stance_duration"].append(stance_dur)
                                self.metrics["swing_duration"].append(
                                    time - last_toe_off
                                )
                                self.metrics["duty_factor"].append(
                                    stance_dur / stride_dur
                                )
                ...
                if self.active_stance_start_time[foot] is not None:
                    ...
                    if stance_dur > 0.1:
                        ...
                        if 0.15 < v_belt_inst < 4.0 and is_clean:
                            ...
                            self.belt_velocity = np.array(
                                [-d_walk * self.current_belt_speed, 0.0, 0.0]
                            )
